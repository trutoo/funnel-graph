...
export class FunnelGraph {
  ...
  constructor(options: FunnelOptions) {
    ...
    this.data = {
      labels: [],
      colors: getDefaultColors(isLayered(options.data) ? options.data.values[0].length : 1),
      subLabels: [],
      ...options.data,
    };
  ...
  getCrossAxisPoints() {
    ...
    if (isLayered(this.data)) {
      ...
      const values = [...this.data.values].concat([...this.data.values].pop() as number);
  ...
  addLabels() {
    ...
    this.percentages.forEach((percentage, index) => {
      ...
      const valueNumber = isLayered(this.data) ? this.getValues2d()[index] : this.data.values[index];
      ...
      if (isLayered(this.data) && this.data.subLabels?.length) {
        ...
        this.data.subLabels.forEach((subLabel, j) => {
          ...
          const subLabelDisplayValue =
            this.subLabelValue === 'percent' ? `${twoDimPercentages[index][j]}%` : formatNumber(data.values[index][j]);
          percentageList += `
  ...
  getValues2d() {
    ...
    this.data.values.forEach((valueSet) => {
  ...
  applyGradient(svg: Element, path: Element, colors: string[], index = 0) {
  ...
  // todo: only re-render when data changes
  updateData(data: Partial<FunnelData | FunnelDataLayered>) {
    if (!this.container) return;
    ...
    this.data = {
      ...
      labels: [],
      subLabels: [],
      colors: getDefaultColors(isLayered(data) ? data.values[0].length : 1),
      ...data,
    };
    this.makeSVG();
    this.drawPaths();
